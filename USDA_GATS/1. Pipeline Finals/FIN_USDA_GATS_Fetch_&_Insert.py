##          This script is designed to take a years input of a specific year and access the GATS data from the USDA website          ##
...
##### Choose table to download into - OPTIONS 
table = "test_table"
...
##### Step 4: Update SQL Database Table
...
##### Step 5: Create Connection to Yearly Export Data
def get_yearly_export_data(partner_code: str, year: str, api_key: str) -> pd.DataFrame:
    ...
    for month_num in range(1, 13):
        ...
        while response.status_code == 429:
            print(f"\n⌛  Rate limit hit for {partner_code}, {month}/{year}. Waiting 2 minutes...")
            time.sleep(120)
            response = requests.get(url)
...
##### Step 6: Insert Each Country Data  
for year in years:
...
##### Step 7: Close Everything & Final Prints 
conn.commit()
cursor.close()
conn.close()
